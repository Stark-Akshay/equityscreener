...
import { ChartDataPoint, DashboardModalProps, StockPriceData } from '../types/types';
import { X, Loader2, RefreshCw } from 'lucide-react';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts';
import { getStockPricesUrl } from '../utils/apiIUtils';
...
export default function DashboardModal({ isOpen, onClose, selectedItems }: DashboardModalProps) {
    ...
    const fetchPriceData = useCallback(async (isRetry = false) => {
        ...
        try {
            ...
            const response = await fetch(getStockPricesUrl(), {
                method: 'POST',
                headers: {
            ...
            <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                ...
                >
                    ...
                    <header className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200">
                        ...
                        <button
                            onClick={onClose}
                            className="text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors"
                    ...
                    <main className="p-4 sm:p-6">
                        {isLoading ? (
                            <div
                                className={`flex flex-col items-center justify-center h-64 sm:h-96 transition-all duration-300 ease-in-out
                                    ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
                                aria-live="polite"
                                aria-busy="true"
                            >
                                <Loader2 size={32} className="animate-spin text-blue-500 sm:hidden" aria-hidden="true" />
                                <Loader2 size={40} className="hidden sm:block animate-spin text-blue-500" aria-hidden="true" />
                            ...
                            <div
                                className={`p-4 rounded-md transition-all duration-300 ease-in-out ${retryAfter !== null ? 'bg-yellow-50 text-yellow-800' : 'bg-red-50 text-red-600'
                            ...
                            >
                                ...
                                {retryAfter !== null ? (
                                    ...
                                    <button
                                        ...
                                        className="mt-2 text-sm font-medium underline hover:no-underline focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
                                    >
                                        Try again
                                    </button>
                                )}
                            ...
                            <div
                                className={`h-64 sm:h-96 lg:h-[500px] transition-all duration-500 ease-out
                            ...
                            <div
                                className="text-center text-gray-500 h-64 sm:h-96 flex items-center justify-center"
                                aria-live="polite"
                            >
                        ...
                        <div
                            className={`mt-6 sm:mt-8 transition-all duration-500 ease-out
                                ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
                            style={{
                                transitionDelay: isOpen ? '400ms' : '0ms'
                            }}
                        >
                            <h3 id="selected-stocks-heading" className="text-sm font-medium text-gray-700 mb-3">Selected Stocks:</h3>
                            <div
                                className="flex flex-wrap gap-2"
                                aria-labelledby="selected-stocks-heading"
                                role="list"
                            >
                                {selectedItems.map((item, index) => (
                                    ...
                                    >
                                        ...
                                        {priceData.find(p => p.symbol === item["1. symbol"])?.error && (
                                            <span className="ml-2 text-red-500" aria-label="Error loading data">
                                                (Error)
                                            </span>
                                        )}
                                    </div>
                    ...
                    {/* Footer */}
                    <footer
                        className={`px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-200 flex flex-col sm:flex-row sm:justify-between items-center gap-2 sm:gap-0 transition-all duration-500 ease-out
                            ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
                        style={{
                            transitionDelay: isOpen ? '400ms' : '0ms'
                        }}
                    >
                        <button
                            onClick={() => fetchPriceData()}
                            className="w-full sm:w-auto flex items-center justify-center px-4 py-2 bg-blue-100 text-blue-700 rounded-md hover:bg-blue-200 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                            disabled={isLoading || isRetrying}
                            aria-busy={isLoading || isRetrying}
                        >
                            <RefreshCw size={16} className={`mr-2 ${isLoading ? 'animate-spin' : ''}`} aria-hidden="true" />
                            <span className="text-sm sm:text-base">Refresh Data</span>
                        </button>
                        <button
                            onClick={onClose}
                            className="w-full sm:w-auto px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
                        >
